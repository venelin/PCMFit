...
```
- [[PCMBaseCpp](https://github.com/venelin/PCMBaseCpp)]. This package provides fast Rcpp modules for likelihood calculation of the model types implemented in PCMBase. PCMBaseCpp can be used as a companion and not a substitute of PCMBase. The sole purpose of PCMBaseCpp is to speed-up the likelihood calculation by implementing the most computationally intensive algorithms in C++, rather than R [@Mitov:2018fl]. Hence, installing this package is optional but highly recommended, in particular, if the goal is to infer models with shifts and/or to infer models on trees bigger than 100 tips. Currently, PCMBaseCpp can be installed from github using the command:
```{r, eval=FALSE}
